...
const SUPABASE_JSON_URL = 'https://ecrvpzlxargnrywftrwe.supabase.co/storage/v1/object/public/news//noticias-ia-sv.json';
const PLACEHOLDER_IMG = 'https://via.placeholder.com/320x200?text=AI+News';
...
export default function AINews() {
  ...
  const newsWithImages = (news || []).map(n => ({
    ...n,
  ...
  return (
    <div className="ain-news-list-modern">
      {newsWithImages.map((item, idx) => (
        <div key={idx} className="ain-news-card-modern">
          <img
            src={item.image}
            alt={item.title}
            onError={e => { (e.currentTarget as HTMLImageElement).src = PLACEHOLDER_IMG; }}
            className="ain-news-img-modern"
          />
